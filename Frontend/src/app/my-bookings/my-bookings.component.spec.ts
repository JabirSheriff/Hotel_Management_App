...
// Interfaces (unchanged from your provided AuthService)
...
export class MyBookingsComponent implements OnInit, OnDestroy {
  ...
  async fetchBookings(): Promise<void> {
    ...
    try {
      const rawBookings = await this.http.get<RawBooking[]>('http://localhost:5280/api/booking/customer', { headers }).toPromise();
      ...
      const bookings: Booking[] = rawBookings.map(b => ({
        ...
        roomsBookedCount: b.roomsBookedCount,
  ...
  async viewDetails(bookingId: number): Promise<void> {
    ...
    try {
      ...
      console.log('Raw booking details from API:', JSON.stringify(booking, null, 2)); // Debug log
      if (!booking) {
      ...
      if (!currentUser) {
        ...
        // Map room types with null/undefined check
        const rooms: Room[] = booking.roomsBooked ? booking.roomsBooked.map(room => ({
          roomId: room.roomId,
